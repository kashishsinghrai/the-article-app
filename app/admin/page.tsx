...
const AdminPage: React.FC<AdminPageProps> = ({
  ...
  const handleToggleAdmin = async (userId: string, currentRole: string) => {
    ...
    if (!window.confirm(currentRole === "admin" ? revokeText : confirmText))
      return;
            ...
            <div className="lg:col-span-4 bg-slate-950 rounded-[2.5rem] p-8 md:p-12 border border-white/10 relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="font-mono text-[11px] space-y-2 text-slate-400">
                  <p>
                    <span className="text-emerald-500">[OK]</span> Handshake
                    protocol validated.
                  </p>
                  <p>
                    <span className="text-blue-500">[INFO]</span> Root admin
                    session established from authorized range.
                  </p>
                  <p>
                    <span className="text-amber-500">[WARN]</span> Accessing
                    restricted member metadata ledger...
                  </p>
                  <p>
                    <span className="text-slate-600">
                      {" >> "} Decrypting operator contact protocols...
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-50 dark:divide-slate-900">
                        {filteredUsers.map((user) => (
                          <tr
                            key={user.id}
                            className="transition-colors hover:bg-slate-50/50 dark:hover:bg-white/5 group"
                          >
                            ...
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-2">
                                ...
                                <p className="text-[10px] font-black text-slate-600 dark:text-slate-400 font-mono tracking-tighter">
                                  {user.serial_id}
                                </p>
                              </div>
                            </td>
                ...
                <div className="grid grid-cols-1 gap-6 xl:hidden md:grid-cols-2">
                  {filteredUsers.map((user) => (
                    <div
                      key={user.id}
                      className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6 group hover:border-red-500/30 transition-all"
                    >
            ...
            {activeTab === "articles" && (
              <div className="grid grid-cols-1 gap-6 pb-20 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredArticles.map((article) => (
                  <div
                    key={article.id}
                    className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2.5rem] shadow-sm flex flex-col justify-between group hover:border-blue-500/30 transition-all"
                  >
                    <div className="space-y-6">
                      <div className="flex items-start justify-between">
                        <span className="text-[9px] font-black uppercase tracking-widest text-blue-600 px-4 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-full">
                          {article.category}
                        </span>
                        <button
                      ...
                      <h3 className="text-2xl italic font-black leading-tight tracking-tighter uppercase dark:text-white line-clamp-2">
                        {article.title}
                      </h3>
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-400">
                          ID
                        </div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                          Op: {article.author_name}
                        </p>
                      </div>
                    </div>
            ...
            <div className="space-y-8 lg:col-span-5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] font-black text-white bg-red-600 px-3 py-1 rounded-full uppercase tracking-widest">
                  {intercepts.length} Active Waves
                </span>
              </div>
                  ...
                  {intercepts.map((i, idx) => (
                    <button
                      ...
                      className={`w-full p-8 rounded-[2.5rem] border transition-all flex justify-between items-center text-left ${
                        activeInterception === i.room
                          ? "bg-red-600 border-red-600 text-white shadow-2xl scale-[1.02] z-10"
                    ...
                    >
                      <div className="space-y-2 overflow-hidden">
                        ...
                        >
                          Signal Hash: {i.room.substring(5, 12)}
                        </p>
                        <div className="flex items-center gap-4">
              ...
              <div className="flex items-center justify-between pb-8 mb-8 border-b border-white/10">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-mono text-slate-500 uppercase">
                    CHANNEL_ID: {activeInterception?.substring(5, 16) || "NULL"}
                  </span>
                  <span className="text-[8px] font-black text-red-500 uppercase tracking-widest pt-1">
                    Decryption: Active
                  </span>
                </div>
              </div>
              ...
              >
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[9px] font-mono text-slate-600 uppercase italic">
                        Packet: {new Date(m.timestamp).toLocaleTimeString()}
                      </span>
                    </div>
                ...
                {!activeInterception && (
                  <div className="flex flex-col items-center justify-center h-full py-24 text-center opacity-20">
                    ...
                    <p className="text-[11px] font-black uppercase tracking-[0.5em] text-white max-w-[300px] leading-loose">
                      SELECT A PULSE NODE FROM THE REGISTRY TO START DECRYPTION
                    </p>
                  </div>
                )}
...
const MonitorCard = ({ label, value, color, desc }: any) => (
  <div className="p-10 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] space-y-4 shadow-sm hover:shadow-2xl hover:scale-[1.02] transition-all group">
    ...
    <div className="flex items-center gap-2 pt-2">
      ...
      <p className="text-[10px] font-bold text-slate-400 italic tracking-wide uppercase">
        {desc}
      </p>
    </div>
  </div>
