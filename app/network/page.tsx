...
const NetworkPage: React.FC<NetworkPageProps> = ({
  ...
  // Auto-sync on mount to ensure we have the latest registry
  useEffect(() => {
    ...
    <main className="max-w-6xl px-6 py-16 mx-auto space-y-12 md:py-32">
      <section className="space-y-8">
        ...
        <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-end">
          ...
          <button
            ...
            className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 text-slate-600 dark:text-white shadow-sm flex items-center justify-center gap-2 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all min-w-[160px]"
          >
            {isRefreshing ? (
        ...
        <div className="flex items-center gap-4 px-6 py-4 transition-all border bg-slate-50 dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800 focus-within:ring-1 focus-within:ring-blue-600">
          <Search size={18} className="text-slate-400" />
          <input
            type="text"
            placeholder="Locate by Name, Handle or Serial ID..."
            value={searchTerm}
      ...
      <section className="grid grid-cols-1 gap-8 pb-20 sm:grid-cols-2 lg:grid-cols-3">
        {filteredUsers.length === 0 ? (
          <div className="py-40 text-center col-span-full opacity-30">
            <Fingerprint size={48} className="mx-auto mb-4 text-slate-300" />
              ...
              >
                <div className="space-y-6">
                  ...
                  <p className="h-8 text-xs italic font-medium leading-relaxed text-slate-500 line-clamp-2">
                    "{user.bio || "Verified correspondent node."}"
                  </p>
                </div>
